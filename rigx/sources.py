...
def compute_target_files(project: Project, target: Target) -> list[str]:
    ...
    # scope (a reference into a sibling `custom` target's `$out`, like
    # `${gen}/foo.nim`), not paths on disk. They contribute nothing to
    # the on-disk src filter and need to be excluded from the
    ...
    keep: set[str] = set(on_disk_sources)
